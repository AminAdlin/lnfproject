...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="max-w-4xl mx-auto px-6 py-8">
        ...
        @if (session('status'))
            <div class="mb-6 text-sm text-green-700 bg-green-50 border border-green-200 p-4 rounded-2xl">
                ✅ {{ session('status') }}
            </div>
        @endif
        ...
        @if($claims->count() > 0)
            <div class="space-y-4">
                @foreach($claims as $claim)
                    <div class="bg-white rounded-2xl shadow border border-red-50 p-5">
                            ...
                            <div class="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">
                                        🚚 {{ $claim->delivery_method === 'self_pickup' ? 'Self Pickup' : 'Delivery' }}
                                    </p>
                                </div>
                            </div>
                            <div>
        ...
        @else
            <div class="text-center py-24 bg-white rounded-2xl shadow border border-red-50">
                ...
                <a href="/items" class="bg-red-800 hover:bg-red-900 text-white text-sm font-semibold py-2 px-6 rounded-xl transition">
                    Browse Items
                </a>
            </div>
        @endif
